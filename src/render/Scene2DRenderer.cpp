    ...
    GpuTexture& out_tex)
{
    ...
    result = vkQueueSubmit(vulkan_context_->GetQueue(), 1, &submit, VK_NULL_HANDLE);
    VulkanContext::CheckVkResult(result);
    vkQueueWaitIdle(vulkan_context_->GetQueue());
    ...
    bool use_pak_streaming)
{
    ...
    for (const std::string& line : lines)
    {
        ...
        for (unsigned char ch : line)
        {
            ...
            line_w += static_cast<int>(std::ceil((advance + kern) * scale));
            prev_codepoint = ch;
        }
        total_width = (std::max)(total_width, line_w);
    }

    const int bitmap_width = (std::max)(1, total_width + 2);
    const int bitmap_height = (std::max)(1, static_cast<int>(lines.size()) * line_height_px + 4);

    std::vector<unsigned char> bitmap(static_cast<std::size_t>(bitmap_width) * static_cast<std::size_t>(bitmap_height), 0u);
    ...
    for (int line_index = 0; line_index < static_cast<int>(lines.size()); ++line_index)
    {
        ...
        float pen_x = 1.0f;
        const int baseline_y = line_index * line_height_px + static_cast<int>(std::ceil(ascent * scale));
        int prev_codepoint = 0;
        for (unsigned char ch : line)
        {
            ...
            {
                std::vector<unsigned char> glyph_bitmap(static_cast<std::size_t>(glyph_w) * static_cast<std::size_t>(glyph_h));
                stbtt_MakeCodepointBitmap(&font_info, glyph_bitmap.data(), glyph_w, glyph_h, glyph_w, scale, scale, ch);

                const int dest_x = static_cast<int>(std::round(pen_x)) + x0;
                const int dest_y = baseline_y + y0;

                for (int row = 0; row < glyph_h; ++row)
                {
                    for (int col = 0; col < glyph_w; ++col)
                    {
                        const int px = dest_x + col;
                        const int py = dest_y + row;
                        if (px >= 0 && px < bitmap_width && py >= 0 && py < bitmap_height)
                        {
                            const unsigned char src_alpha = glyph_bitmap[static_cast<std::size_t>(row) * static_cast<std::size_t>(glyph_w) + static_cast<std::size_t>(col)];
                            unsigned char& dst = bitmap[static_cast<std::size_t>(py) * static_cast<std::size_t>(bitmap_width) + static_cast<std::size_t>(px)];
                            dst = static_cast<unsigned char>((std::max)(static_cast<unsigned>(dst), static_cast<unsigned>(src_alpha)));
                        }
                    }
                }
            }
    ...
    GpuTexture tex{};
    const bool ok = UploadTexture(bitmap.data(), bitmap_width, bitmap_height, true, tex);
    if (!ok)
    {
        return nullptr;
    }
    ...
    float r, float g, float b, float a)
{
    ...
    const Vertex2D verts[4] = {
        {ndc_x0, ndc_y0, 0.0f, 0.0f}, // top-left
        {ndc_x1, ndc_y0, 1.0f, 0.0f}, // top-right
        {ndc_x1, ndc_y1, 1.0f, 1.0f}, // bottom-right
        {ndc_x0, ndc_y1, 0.0f, 1.0f}, // bottom-left
    };
    ...
    std::uint32_t height)
{
    ...
    submit.commandBufferCount = 1;
    submit.pCommandBuffers = &cmd;
    vkQueueSubmit(vulkan_context_->GetQueue(), 1, &submit, VK_NULL_HANDLE);
    vkQueueWaitIdle(vulkan_context_->GetQueue());
    vkFreeCommandBuffers(device, command_pool_, 1, &cmd);
}
